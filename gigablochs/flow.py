...
def holdsworth_cca(start=0, stop=2, num=1000,
                   interbeat_interval=0.917, cross_section=None,
    ...
    based on the model by Holdsworth et al. (1999) :cite:t:`Holdsworth1999`.
    ...
    interbeat_interval : float, optional
        The interval between heartbeats (the pulse period) in seconds. Default is 0.917.
    cross_section : array_like, optional
        The normalized flow velocity dependence across the vessel cross-section.
    systolic_velocity : array_like, optional
        The peak systolic velocity in m/s. Default is 0.76 following Zhao et al. (2016) :cite:t:`Zhao2016`.
    diastolic_velocity : array_like, optional
        The diastolic velocity in m/s. Default is 0.3 following Zhao et al. (2016) :cite:t:`Zhao2016`.
    **kwargs : dict
        Additional keyword arguments to pass to the `integrate_trajectory` function.
    ...
    in the CCA, averaged across subjects, taken from Table 2 in Holdsworth et al. (1999) :cite:t:`Holdsworth1999`.
...
def speed_across_vessel(normalized=False, num=1000, time=0, R=0.003, P=13330, N=4, f=1/0.917, mu=3.5e-3, rho=1060):
    ...
    velocity_profile = P0 / (4 * mu) * (R ** 2 - r.astype(np.complex128) ** 2)
    for n in range(1, N):
        alpha_n = a * np.sqrt(n) * 1j ** (3/2)
        coef = (-1j * P) / (rho * n * 2 * np.pi * f)
        cycle = np.exp(1j * n * 2 * np.pi * f * time)
        P /= 2
        velocity_profile = velocity_profile + coef * cycle * (1 - spec.jv(0, alpha_n * r / R) / spec.jv(0, alpha_n))
    r = np.squeeze(r)
    if normalized:
        return r, velocity_profile.real / velocity_profile.real.max()
    return r, velocity_profile.real
