...
def holdsworth_cca(start=0, stop=2, num=1000, interbeat_interval=0.917,
    ...
    based on the model by Holdsworth et al. (1999).
    ...
    interbeat_interval : float, optional
        The interval between heartbeats (the pulse period) in seconds. Default is 0.917.
    systolic_velocity : float, optional
        The peak systolic velocity in m/s. Default is 0.76 following Zhao et al. (2016).
    diastolic_velocity : float, optional
        The diastolic velocity in m/s. Default is 0.3 following Zhao et al. (2016).
    **kwargs : dict
        Additional keyword arguments to pass to the `integrate_trajectory` function.
    ...
    in the CCA, averaged across subjects, taken from Table 2 in Holdsworth et al. (1999). Additional average
    interpolated points are estimated and included in the model. A cubic spline `scipy.interpolate.CubicSpline`
    is fit to the data points, with a periodic boundary condition.
